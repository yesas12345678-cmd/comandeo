...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      // Cargar camareros
      const resWaiters = await fetch('/api/admin/waiters');
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col min-h-screen bg-slate-900 text-slate-100 items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mb-4"></div>
        <p className="text-slate-400 font-semibold">Cargando Panel de Administración...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <header className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex items-center justify-between shadow-lg">
        ...
        <a
          href="/"
          className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-slate-950 font-black rounded-lg text-sm transition-all"
        >
          Ir a la PDA
        </a>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-6">
        ...
        {errorMsg && <div className="p-3 bg-rose-500/20 text-rose-400 rounded-lg text-sm mb-6 font-bold">{errorMsg}</div>}
              ...
              <form onSubmit={handleAddProduct} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950 border border-slate-850 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-blue-500 outline-none font-bold"
                  >
                    {categories.map((cat) => (
                    ...
                    <div key={prod.id} className="flex items-center justify-between p-3 bg-slate-950/40 border border-slate-850/60 rounded-xl">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <button
                          ...
                          className="px-2 py-1 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 rounded text-xs transition-colors font-bold"
                        >
                          Eliminar
                        </button>
                      </div>
